...
def getXYcoor(iuv, resolution = 256):
    ...
    uv_mask[np.ceil(v).astype(np.int64),np.ceil(u).astype(np.int64)]=1
    uv_mask[np.floor(v).astype(np.int64),np.floor(u).astype(np.int64)]=1
    uv_mask[np.ceil(v).astype(np.int64),np.floor(u).astype(np.int64)]=1
    uv_mask[np.floor(v).astype(np.int64),np.ceil(u).astype(np.int64)]=1
...
def mapper(iuv, resolution=256):
    ...
    H, W, _ = iuv.shape
    iuv_raw = iuv[iuv[:, :, 0] > 0]
    x = np.linspace(0, W-1, W).astype(np.int64)
    y = np.linspace(0, H-1, H).astype(np.int64)
