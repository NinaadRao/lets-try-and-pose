...
for i in range(29):
    for j in range(29):
        ...
        pimg, gimgs, oimgs, gen_img, pose = dress_in_order(model, pid=pid, gids=gids, pose_id=pose_id)
        gen_img = gen_img.float().cpu().detach().numpy()
        fig = plt.figure(figsize=(6,4), dpi= 100, facecolor='w', edgecolor='k')
        plt.axis('off')
        plt.imshow(gen_img.transpose(1,2,0))
        print(np.max(gen_img),np.min(gen_img))
        plt.imsave('output_pipeline/'+str(i)+'_'+str(j)+'.png',(gen_img.transpose(1,2,0)+ 1)/2)
