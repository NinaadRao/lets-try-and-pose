...
for i in range(29):
    for j in range(29):
        ...
        plt.imsave('output/'+str(i)+'_'+str(j)+'.png',(gen_img.transpose(1,2,0)+ 1)/2)
...
# # pose_id = ("print", 1, None) # load the 2-nd person from "print" group, NONE (no) garment is interested
# # generate
# pimg, gimgs, oimgs, gen_img, pose = dress_in_order(model, pid, pose_id=pose_id)
# print(gen_img)
# fig = plt.figure(figsize=(6,4), dpi= 100, facecolor='w', edgecolor='k')
...
# plt.imshow(gen_img.transpose(1,2,0))
# print(np.max(gen_img),np.min(gen_img))
# plt.imsave('output.png',(gen_img.transpose(1,2,0)+ 1)/2)
